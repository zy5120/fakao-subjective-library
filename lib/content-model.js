...
function makeCompleteAnswer(record) {
  ...
  const application = `紧扣题面已经给出的事实：“${record.questionSummary}”逐项判断。把每一项事实放入相应构成要件：能够直接满足要件的，明确写明成立；事实不足的，不补造事实，而分别写出“若……则……”的条件式结论。`;
  const branches = "如公开回忆或教师解析存在不同观点，先写通说或更有评分可能的观点，再说明另一观点的规范理由。无论采用何种观点，前提、规则、涵摄与结论必须一致。";
  return {
    conclusion,
    rules,
    application,
...
function finalize(record) {
  return {
    ...record,
    completeQuestion: `${record.questionSummary}\n\n作答边界：除题面明确事实外，不自行假定其他事实；公开回忆材料存在缺口时，分别作条件式判断。`,
    trainingQuestions: questionPrompts(record.subject, record.topic),
    completeAnswer: makeCompleteAnswer(record),
