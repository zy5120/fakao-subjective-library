...
def rule_text(subject, topic):
    ...
    if "行政征收" in combined:
        rules.append("行政征收是行政机关依法、强制且有偿取得财产或收取费用的行为，不以制裁违法为目的。收费对象、范围和标准必须有上位法依据；下位规范擅自扩大缴费义务人的，法院不能据其支持征收决定。")
    if "规范性文件" in combined:
        rules.append("行政诉讼中可对被诉行政行为所依据的规范性文件一并进行附带审查。法院认为规范性文件不合法的，不作为认定行政行为合法的依据，并可向制定机关提出处理建议；规范性文件本身不是独立诉讼标的。")
    if "起诉期限" in combined:
        rules.append("行政机关作出行政行为时未告知起诉期限的，起诉期限自当事人知道或者应当知道起诉期限之日起计算，但自知道或者应当知道行政行为内容之日起最长不得超过一年。诉讼中补充的证据原则上不能倒推证明行政行为作出时已经合法。")
    ...
    if "不可抗力" in combined or "法定解除" in combined:
        rules.append("不可抗力只在不能预见、不能避免且不能克服，并实际影响履行的范围内免责；只有因不可抗力致使合同目的不能实现时，才产生法定解除权。合同解除不等于合同自始无效，约定解除还须有当事人合意或成就约定条件。")
    if "债权转让" in combined or "债务承担" in combined:
        rules.append("债权转让通常不以债务人同意为生效要件，但未经通知对债务人不发生效力；债务人将债务全部或部分转移给第三人，应经债权人同意。作答时应分别判断内部协议效力、对相对人的效力和原债务人是否脱责。")
