...
<body>
  ...
  <script>
    ...
    function render(focusId) {
      document.getElementById("app").innerHTML = header()+'<div class="site-frame">'+body()+'</div><footer><div><span class="brand-seal">法</span><p><strong>法考主观题资料库</strong><small>持续更新至 2026 年 10 月 18 日</small></p></div><p>公开学习整理 · 原题与公布答案请从来源页核对 · 核验截止 2026-09-05</p></footer><nav class="mobile-nav" aria-label="移动端主导航">'+mobileNavItems.map((item) => '<button data-view="'+item[0]+'" class="'+(state.view === item[0] ? "active" : "")+'"><span>'+item[3]+'</span>'+(item[0] === "daily" ? "题库" : item[2])+'</button>').join("")+'</nav>';
...
const assets = [
  [path.join(root, "output/pdf/法考主观题十年与2026每日一题阅读册.pdf"), "downloads/法考主观题十年与2026每日一题阅读册.pdf"],
